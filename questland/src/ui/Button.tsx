...
type SizeSpec = { padding: string; height: number; font: string; icon: number };
...
type VariantState = React.CSSProperties;
type VariantSpec = {
  background: string; color: string; border: string; boxShadow: string;
  hover: VariantState; press: VariantState;
};
...
export function Button({
  ...
  const { hover: hoverStyle, press: pressStyle, ...base } = VARIANTS[variant] || VARIANTS.primary;
  const state = disabled ? {} : press ? pressStyle : hover ? hoverStyle : {};
